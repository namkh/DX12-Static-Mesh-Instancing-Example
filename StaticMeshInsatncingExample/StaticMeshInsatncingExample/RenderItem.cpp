...
bool RenderItem::Initialize(SimpleRenderObject* renderObj,
							...
							uint32_t frameCount)
{
	...
	m_updateCommandBuffers.Initialize();
...
{
	m_writedUpdateCommandBuffers.clear();
}

void RenderItem::Destroy()
{
	m_renderCommandBuffers.Destory();
	m_updateCommandBuffers.Destory();
...
void RenderItem::PopUpdateCommandBuffers(std::vector<CommandBuffer*>& cmdBuffers)
{
	cmdBuffers.insert(cmdBuffers.end(), m_writedUpdateCommandBuffers.begin(), m_writedUpdateCommandBuffers.end());
	m_writedUpdateCommandBuffers.clear();
}

bool RenderItem_StaticMeshInstancing::Initialize(SimpleRenderObject* renderObj,
												 StructuredBuffer<GlobalConstants>* globalConstantBuffer,
												 uint32_t frameCount)
{
	if (renderObj != nullptr)
	...
	return RenderItem::Initialize(renderObj, globalConstantBuffer, frameCount);
}

void RenderItem_StaticMeshInstancing::Destroy()
{
...
void RenderItem_StaticMeshInstancing::UpdateResources()
{
	RenderItem::UpdateResources();

	if (m_renderObj != nullptr)
	{
		CommandBuffer* cmdBuf = m_updateCommandBuffers.GetCommandBuffer();
		...
		if (numInstances > 0 && numInstances == m_instanceDatas.size())
		{
			...
			{
				SampleRenderObjectInstance* instance = m_renderObj->GetInstance(i);
		...
		cmdBuf->End();
		m_writedUpdateCommandBuffers.push_back(cmdBuf);
	}
}

bool RenderItem_StaticMeshInstancing::CreateBuffers()
{
	if (m_renderObj != nullptr)
	{
		uint32_t numInstances = m_renderObj->GetInstanceCount();
...
bool RenderItem_StaticMeshInstancing::CreateDescriptors()
{
	if (m_renderObj == nullptr)
	{
...
int RenderItem_StaticMeshInstancing::GetMaterialBindIndex(SimpleMaterial* material)
{
...
int RenderItem_StaticMeshInstancing::GetTextureBindIndex(SimpleTexture2D* texture)
{
	if (texture != nullptr)
	{
					...
					if (cmdBuf != nullptr)
					{
						...
						for (uint32_t j = 0; j < geometry->GetMeshCount(); j++)
						{
							...
							if (mesh != nullptr)
							{
								...
								uint32_t packCount = min(MESHLET_MAX_VERTS / mesh->GetMeshletInfo().LastMeshletVertIdxCount,
														 MESHLET_MAX_PRIMS / mesh->GetMeshletInfo().LastMeshletPrimCount);
